...
3. Work inside the running container.
...
4. The host repository is synced one-way into `/workspace` inside the devcontainer. When the latest contents of the host repository need to overwrite `/workspace` inside the container, run `bash /repo-ro/test/devcontainer/init-workspace.sh`.

5. Surface blockers precisely.
...
7. Summarize the environment state.
   State which config was used, the command that started the environment, whether follow-up commands ran inside the container, whether `/workspace` was resynced, what cleanup was performed, and whether the repository was returned to a clean post-test state.
