...
After this change, the published Ubuntu image remains a real runtime artifact that runs `bootstrap -> install -> stow -> post` during `docker build`, then converts repository-backed Dotter symlinks in the container home directory into ordinary files and directories before the image is finalized. The finalization step also copies the deployed zsh module directory from `/root/.config/zsh/conf.d` to `/root/conf.d`, which matches the `${PROFILE_ROOT}/conf.d` loading rule used by `/root/.zshrc` after the repository workspace is removed. In addition, the repository gains a machine-readable fact file that explains which repository paths belong to the full user workflow, which belong specifically to the container view of that workflow, and which additionally affect the release-image manifest. A maintainer can verify the result by editing representative files such as `packages/alacritty/...`, `packages/zsh/...`, or `ci/image/Dockerfile` and observing that the new classifiers answer the three questions consistently. Automatic push builds are limited to `master`, so ordinary feature branch pushes no longer consume multi-architecture image build capacity or publish branch tags. A `master` push that contains more than one commit is still classified correctly because the workflow passes GitHub's push `before` SHA and checked-out `sha` to the classifier.
...
- [x] (2026-05-03T08:58:35+08:00) Re-read the release-image finalization script, release image docs, and this ExecPlan before implementing the zsh runtime layout fix.
- [x] (2026-05-03T08:58:35+08:00) Updated `ci/image/finalize-image.sh` so finalization copies `/root/.config/zsh/conf.d` to `/root/conf.d` after repository-backed symlinks are materialized and before `/workspace` is removed.
- [x] (2026-05-03T09:14:23+08:00) Validated the zsh runtime layout fix with Linux `bash -n`, `git diff --check -- ci/image docs plans`, a local `docker build -f ci/image/Dockerfile -t ubuntu-dotfile:local .`, filesystem assertions for `/workspace`, `/root/.zshrc`, `/root/.config/zsh/conf.d`, and `/root/conf.d`, and pseudo-TTY zsh loading proof for `gits` and `gitwt`.
...
- Observation: the release image's deployed zsh modules live under `/root/.config/zsh/conf.d`, but `/root/.zshrc` loads `${PROFILE_ROOT}/conf.d`, which resolves to `/root/conf.d` in the finalized image.
  Evidence: the requested runtime fix preserves Dotter deployment under `/root/.config/zsh/conf.d` and derives `/root/conf.d` during `ci/image/finalize-image.sh`.
- Observation: the zsh loading validation proves the git helper functions load from the derived runtime directory, but the existing profile still reports `08-misc.zsh` as failed.
  Evidence: `script -qec 'PROFILE_DEBUG=1 zsh -ic "whence -w gits gitwt"' /dev/null` printed `gits: function` and `gitwt: function` after the build, while also printing `[zsh profile] Failed to load module: 08-misc.zsh`.
...
- Decision: materialize only symlinks under `$HOME` that resolve into `/workspace`, then delete `/workspace`.
  ...
  Date/Author: 2026-04-10 / Codex
- Decision: copy `/root/.config/zsh/conf.d` to `/root/conf.d` in `ci/image/finalize-image.sh` rather than changing `packages/zsh/.zshrc`, setting `ZDOTDIR`, or changing XDG configuration behavior.
  Rationale: this keeps normal Dotter deployment and the user workflow unchanged while making the release-image filesystem match the zsh runtime path that already exists in `/root/.zshrc`.
...
The release image therefore needs one extra step after `just up` completes. That step scans the deployed files in the container home directory, finds the symbolic links that still point back into the build workspace, replaces them with ordinary copied files or directories, creates `/root/conf.d` by copying the deployed zsh modules from `/root/.config/zsh/conf.d`, and only then removes the workspace. This happens only for the release-image build, not for the general `stow` implementation.
...
For the 2026-05-03 zsh runtime fix, update only `ci/image/finalize-image.sh` and release-image documentation. After the existing repository-backed symlink materialization loop has completed and before deleting `/workspace`, run `cp -aL "$HOME_DIR/.config/zsh/conf.d" "$HOME_DIR/conf.d"`. Do not add a fallback or existence guard: if Dotter did not deploy `/root/.config/zsh/conf.d`, the image build should fail and expose the broken build pipeline. Do not pre-delete `/root/conf.d`; the release image is expected to build from a clean Ubuntu base where that path is absent.
...
12. For the 2026-05-03 zsh runtime fix, update `ci/image/finalize-image.sh` after the remaining-link check so it copies `/root/.config/zsh/conf.d` to `/root/conf.d`, update `docs/release-image.md` and `ci/image/README.md` to document the finalized zsh layout, and validate with `bash -n ci/image/finalize-image.sh`, `git diff --check -- ci/image docs plans`, and a local release-image build.

## Validation and Acceptance

Acceptance for the release image remains behavioral. A successful local build of `ci/image/Dockerfile` must complete `bootstrap-up.sh` during `docker build`, and a container started from that image must contain deployed dotfiles under `/root` as ordinary files and directories rather than symlinks into `/workspace`. The final image must not retain `/workspace`. It must contain `/root/.zshrc`, `/root/.config/zsh/conf.d`, and `/root/conf.d`; `/root/conf.d` must be a normal directory copied during image finalization, and `PROFILE_DEBUG=1 zsh -ic "whence -w gits gitwt"` should show that the expected zsh functions load in an interactive shell.
...
- The finalized release image derives `/root/conf.d` from `/root/.config/zsh/conf.d`; normal Dotter deployment still owns the source zsh module directory.
...
Revision note: updated on 2026-05-03 to record the release-image zsh runtime layout fix that copies Dotter-deployed zsh modules into `/root/conf.d` during finalization.
